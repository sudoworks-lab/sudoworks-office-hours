...
test("frontend exposes the required product, state, and accessibility landmarks", async () => {
  ...
  assert.match(html, /技術対話リクエスト機能の実装デモ/u);
  assert.match(html, /運営者への自動通知、返信保証、面談設定、カレンダー登録は行いません/u);
  assert.match(html, /ローカル評価モードの既定の開発用キーは公開情報/u);
  assert.match(html, /リクエストを保存しました/u);
  assert.match(html, /運営者への通知・確認連絡・面談設定・カレンダー登録は自動では行いません。返信も保証しません/u);
  ...
  assert.match(javascript, /保存しています…/u);
  assert.match(javascript, /入力内容を確認してから、リクエストをもう一度保存してください/u);
  assert.match(javascript, /時間枠は別のリクエストで確保されました/u);
  assert.match(javascript, /予期しない問題により、リクエストを保存できませんでした/u);
  assert.match(javascript, /短時間に多くのリクエストを受け付けました/u);
  assert.match(javascript, /リクエストの保存先を一時的に利用できません/u);
